...
export function buildConfigurationSummary(ctx: SummaryBuilderContext): SummaryRow[] {
    ...
    const fileLaunchItems = [
        ctx.gameRootManualOverride ? `${ct('luthier_summary_game_root')}: ${ctx.gameRootRelativeDisplay}` : '',
    ...
    if (!cfg.winecfg.virtual_desktop.state.use_wine_default) {
        winecfgItems.push(
            `${ct('luthier_summary_virtual_desktop')}: ${featureStateEnabled(cfg.winecfg.virtual_desktop.state.state)
