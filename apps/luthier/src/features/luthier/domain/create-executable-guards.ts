...
function hasWindowsLauncherExtension(path: string): boolean {
    ...
    return lower.endsWith('.exe') || lower.endsWith('.bat') || lower.endsWith('.cmd') || lower.endsWith('.com')
}
...
export function getCreateExecutableValidationErrors(ctx: CreateExecutableGuardsContext): string[] {
    ...
    if (currentExePath && rootTrimmed && isLikelyAbsolutePath(currentExePath) && isLikelyAbsolutePath(rootTrimmed)) {
    ...
    cfg.compatibility.wrapper_commands.forEach((item: typeof cfg.compatibility.wrapper_commands[0], index: number) => {
    ...
    cfg.extra_system_dependencies.forEach((item: typeof cfg.extra_system_dependencies[0], index: number) => {
        if (!item.name.trim()) {
            errors.push(
                prefixed(
    ...
    cfg.winecfg.dll_overrides.forEach((item: typeof cfg.winecfg.dll_overrides[0], index: number) => {
    ...
    cfg.winecfg.desktop_folders.forEach((item: typeof cfg.winecfg.desktop_folders[0], index: number) => {
