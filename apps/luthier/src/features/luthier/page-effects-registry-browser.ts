...
export function createLuthierPageRegistryBrowserEffects(
  ...
  const importRegistryKeysFromRegFile = async () => {
    try {
      ...
      controller.setStatusMessage(
        ctf('luthier_failed_to_import_reg_file_error', { error: String(error) })
      )
    }
  }
  ...
  const loadMountBrowserDirs = async (absolutePath: string) => {
    ...
    try {
      const result: ListChildDirectoriesOutput =
        await luthierBackendApi.listChildDirectories(absolutePath)
      setMountBrowserPath(result.path)
      setMountBrowserDirs(result.directories)
    } catch (error) {
      controller.setStatusMessage(
        ctf('luthier_failed_to_list_folders_error', { error: String(error) })
      )
    } finally {
  ...
  const loadIntegrityBrowserEntries = async (absolutePath: string) => {
    ...
    try {
      const result: ListDirectoryEntriesOutput =
        await luthierBackendApi.listDirectoryEntries(absolutePath)
      setIntegrityBrowserPath(result.path)
      setIntegrityBrowserDirs(result.directories)
      setIntegrityBrowserFiles(result.files)
    } catch (error) {
      controller.setStatusMessage(
        ctf('luthier_failed_to_list_files_error', { error: String(error) })
      )
    } finally {
  ...
  const openIntegrityFileBrowser = async () => {
    ...
    if (!root) {
      controller.setStatusMessage(
        ct('luthier_select_an_executable_first_to_define_the_game_folder')
      )
      return null
    }
