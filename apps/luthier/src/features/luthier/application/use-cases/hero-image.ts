...
export function createHeroImageUseCase({ backend, notifier, state, messages }: HeroImageUseCaseDeps) {
    ...
    const searchHeroImageAutomatically = async () => {
        ...
        if (
            ...
            const currentIndex = initialState.searchCandidates.findIndex((candidate) => candidate === currentUrl)
