...
export function createLuthierComputed(state: ReturnType<typeof createLuthierState>) {
    ...
    const gamescopeEnabled = createMemo(() => isFeatureEnabled(state.config().environment.gamescope.state))

    const availableFallbackCandidates = createMemo(() =>
        RUNTIME_CANDIDATES.filter((candidate) => candidate !== state.config().requirements.runtime.primary)
    )

    const normalizedWinetricksSearch = createMemo(() => state.winetricksSearch().trim().toLowerCase())
    const normalizedHeroSearchGameName = createMemo(() => state.config().game_name.trim().toLowerCase())

    const winetricksCandidates = createMemo(() => {
        const search = normalizedWinetricksSearch()
        if (search.length < 2) return []

        return state.winetricksAvailable()
    ...
    const createExecutableValidationErrors = createMemo(() => getCreateExecutableValidationErrors({
        config: state.config(),
        locale: state.locale(),
