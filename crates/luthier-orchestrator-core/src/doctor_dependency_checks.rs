...
use super::{CheckStatus, DependencyStatus, RuntimeDiscovery};
...
pub(super) fn evaluate_dependencies(
    ...
    let gamemoderun_bin = super::find_in_path("gamemoderun");
    ...
    let mut out = vec![
        ...
        evaluate_component(
            ...
            super::find_in_path("mangohud"),
        ),
        evaluate_component(
            "winetricks",
            config.map(|cfg| cfg.requirements.winetricks),
            super::find_in_path("winetricks"),
        ),
        evaluate_component("umu-run", config.map(|cfg| cfg.requirements.umu), super::discover_umu()),
    ];
...
fn evaluate_gamemode_umu_runtime_component(
    ...
    let resolved_path = gamemoderun_bin
        .as_ref()
        .map(|p| super::path_to_string(p.clone()))
        .or_else(|| umu_run.as_ref().map(|p| super::path_to_string(p.clone())));
...
fn evaluate_gamemoderun_component(
    ...
    match (binary, libgamemode.is_some()) {
        ...
        (Some(binary_path), false) => {
            ...
            status.resolved_path = Some(super::path_to_string(binary_path));
...
fn find_dependency_from_rules(
    ...
    for command in commands {
        if let Some(path) = super::find_in_path(command) {
            return Some(path);
        }
    }
...
pub(super) fn evaluate_component(
    ...
    DependencyStatus {
        ...
        status,
        found,
        resolved_path: resolved.map(super::path_to_string),
        note: note.to_string(),
    }
}
