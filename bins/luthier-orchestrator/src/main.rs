...
fn log_startup_event(trace_id: &str, cli: &Cli) {
    log_event(
        ...
        serde_json::json!({
            ...
            "out": cli.out.as_ref().map(|v| v.display().to_string()),
            "lang": cli.lang,
...
fn route_explicit_commands(trace_id: &str, cli: &Cli) -> anyhow::Result<bool> {
    ...
    let has_extract_command = cli.extract_config || cli.extract_hero_image || cli.extract_icon;
    if cli.out.is_some() && !has_extract_command {
        return Err(anyhow::anyhow!("--out requires an extract command"));
    }

    if cli.show_manifest {
        run_show_manifest_command(trace_id)
            .context("failed to print embedded asset container manifest")?;
    }

    if cli.extract_config {
        run_extract_config_command(trace_id, cli.out.as_deref())
            .context("failed to extract embedded config asset")?;
    }

    if cli.extract_hero_image {
        run_extract_hero_image_command(trace_id, cli.out.as_deref())
            .context("failed to extract embedded hero image asset")?;
    }

    if cli.extract_icon {
        run_extract_icon_command(trace_id, cli.out.as_deref())
            .context("failed to extract embedded icon asset")?;
    }
